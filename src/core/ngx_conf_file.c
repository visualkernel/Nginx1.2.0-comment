...
ngx_conf_parse(ngx_conf_t *cf, ngx_str_t *filename)
{
    ...
    if (filename) {//正在解析文件
        ...
        buf.pos = buf.start;
        buf.last = buf.start;
        buf.end = buf.last + NGX_CONF_BUFFER;//4096 byte
        buf.temporary = 1;//可以被修改
    ...
    } else if (cf->conf_file->file.fd != NGX_INVALID_FILE) {//正在解析块指令
...
ngx_conf_read_token(ngx_conf_t *cf)
{
    ...
    for ( ;; ) {
        ...
        if (b->pos >= b->last) {//如果没有未处理的字符

            if (cf->conf_file->file.offset >= file_size) {//文件已经读完
